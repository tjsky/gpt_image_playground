...
async function parseImagesApiResponse(payload: ImageApiResponse, mime: string, signal?: AbortSignal, profile?: ApiProfile): Promise<CallApiResult> {
  ...
  try {
    for (const item of data) {
      ...
      if (isHttpUrl(item.url) || isDataUrl(item.url)) {
        const finalUrl = (profile && isHttpUrl(item.url))
          ? proxyImageUrlIfNeeded(item.url, profile)
          : item.url;

        images.push(await fetchImageUrlAsDataUrl(finalUrl, mime, signal))
        revisedPrompts.push(typeof item.revised_prompt === 'string' ? item.revised_prompt : undefined)
      }
    }
  } catch (err) {
...
async function callImagesApiSingle(opts: CallApiOptions, profile: ApiProfile, customProvider?: CustomProviderDefinition | null): Promise<CallApiResult> {
  ...
  try {
    ...
    return parseImagesApiResponse(await response.json() as ImageApiResponse, mime, controller.signal, profile)
  } finally {
